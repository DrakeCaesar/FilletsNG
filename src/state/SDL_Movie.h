...
public:
  ...
  void CheckErrors()
  {
    ...
    if (error)
    {
      LOG_WARNING(ExInfo("SMPEG_error").addInfo("error", error));
    }
  }
  ...
  void Load(const char *fileName, int maxscalex = 1, int maxscaley = 1)
  {
    ...
    SDL_Surface *tempSurface2 = SDL_CreateRGBSurface(
        SDL_SWSURFACE, movieInfo.width * MaxScaleX, movieInfo.height * MaxScaleY, 32, screen->format->Rmask,
