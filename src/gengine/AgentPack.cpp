...
void AgentPack::addAgent(BaseAgent *agent)
{
  std::pair<t_agents::iterator, bool> status =
      m_agents.insert(std::pair<std::string, BaseAgent *>(agent->getName(), agent));
  if (!status.second)
  {
    throw NameException(ExInfo("agent already exists").addInfo("name", agent->getName()));
  }
 ...
 */
BaseAgent *AgentPack::getAgent(const std::string &name)
{
  ...
  {
    throw NameException(ExInfo("cannot find agent").addInfo("name", name));
  }

  if (!it->second->isInitialized())
  {
    throw LogicException(ExInfo("agent is not initialized").addInfo("name", name));
  }
  return it->second;
}
//-----------------------------------------------------------------
...
void AgentPack::shutdown()
{
  ...
  for (t_agents::reverse_iterator i = m_agents.rbegin(); i != rend; ++i)
  {
    if (i->second->isInitialized())
    {
      i->second->shutdown();
