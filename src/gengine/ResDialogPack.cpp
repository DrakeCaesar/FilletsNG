...
//-----------------------------------------------------------------
void ResDialogPack::unloadRes(Dialog *res)
{
  delete res;
}
//-----------------------------------------------------------------
 ...
 */
int ResDialogPack::matchScore(const std::string &first, const std::string &second) const
{
  int score = 0;
 ...
 */
const Dialog *ResDialogPack::findDialog(const std::string &name, const std::string &lang)
{
  int bestScore = 0;
...
const Dialog *ResDialogPack::findDialogHard(const std::string &name)
{
  ...
  if (NULL == dialog)
  {
    ...
    if (NULL == dialog)
    {
      LOG_WARNING(ExInfo("cannot find dialog").addInfo("name", name).addInfo("lang", lang).addInfo("pack", toString()));
    }
  }
...
{
  std::string speech = OptionAgent::agent()->getParam("speech", OptionAgent::agent()->getParam("lang"));
  ...
  if (NULL == dialog || dialog->isSpeechless())
  {
    ...
    {
      LOG_WARNING(
          ExInfo("cannot find speech").addInfo("name", name).addInfo("speech", speech).addInfo("pack", toString()));
    }
  }
