...
Environ::~Environ()
{
	auto end = m_watchers.end();
	for (auto i = m_watchers.begin(); i != end; ++i)
	{
		delete i->second;
	}
}
...
Environ::store(const Path& file)
{
	...
	if (config)
	{
		...
		for (auto i = m_values.begin(); i != end; ++i)
		{
                  ...
                  int implicit) const
{
	...
	bool ok;
	int result = static_cast<int>(StringTool::readInt(value.c_str(), &ok));
	if (!ok)
	{
		if (value != "")
		{
...
{
	auto end = m_watchers.end();
	for (auto i = m_watchers.begin(); i != end; /*empty*/)
	{
		auto cur = i++;
