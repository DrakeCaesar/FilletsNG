...
//-----------------------------------------------------------------
void ResSoundPack::unloadRes(Mix_Chunk *res)
{
  Mix_FreeChunk(res);
}
//-----------------------------------------------------------------
 ...
 */
Mix_Chunk *ResSoundPack::loadSound(const Path &file)
{
  ...
  if (OptionAgent::agent()->getAsBool("sound", true))
  {
    ...
    if (NULL == chunk)
    {
      LOG_WARNING(ExInfo("cannot load sound").addInfo("path", file.getNative()).addInfo("MixError", Mix_GetError()));
    }
  }
  return chunk;
}
 ...
 */
void ResSoundPack::addSound(const std::string &name, const Path &file)
{
