 ...
 * Share resources.
 */
template <class T> class ResourcePack : public INamed
{
public:
...
public:
  ...
  virtual ~ResourcePack()
  {
    if (!m_reses.empty())
    {
      LOG_WARNING(ExInfo("resources are not released").addInfo("pack", toString()));
    }
  }
  //-----------------------------------------------------------------
  /**
  ...
  {
    std::pair<t_resIterator, t_resIterator> range = m_reses.equal_range(name);
  ...
  {
    m_reses.insert(std::pair<std::string, T>(name, res));
  }
  //-----------------------------------------------------------------
  /**
  ...
  T getRes(const std::string &name, int rank = 0)
  {
    ...
    if (range.second == range.first)
    {
      throw ResourceException(
          ExInfo("no such resource at index").addInfo("name", name).addInfo("index", rank).addInfo("pack", toString()));
    }
  ...
  T getRandomRes(const std::string &name)
  {
    ...
    {
      LOG_WARNING(ExInfo("no such resource").addInfo("name", name).addInfo("pack", toString()));
    }
    return result;
  }
  //-----------------------------------------------------------------
  ...
  {
    ExInfo available_res = ExInfo("resources").addInfo("name", getName());
