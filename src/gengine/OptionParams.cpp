...
//-----------------------------------------------------------------
void OptionParams::addParam(const std::string &name, eType type, const std::string &help)
{
  Param param(type, help);
  m_params.insert(std::pair<std::string, Param>(name, param));
...
//-----------------------------------------------------------------
std::string OptionParams::getType(eType type) const
{
  switch (type)
  {
  case TYPE_NUMBER:
  ...
  case TYPE_PATH:
    return "path";
  default:
    LOG_WARNING(ExInfo("unknown param type").addInfo("type", type));
  }
  return "unknown";
}
//-----------------------------------------------------------------
...
void OptionParams::checkValidity(const std::string &name, const std::string & /*value*/) const
{
  ...
  if (m_params.end() == it)
  {
    throw LogicException(ExInfo("unknown option").addInfo("name", name));
  }
