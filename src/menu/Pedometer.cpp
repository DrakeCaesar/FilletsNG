...
Pedometer::Pedometer(LevelStatus *status, Level *new_level)
{
  ...
  m_numbers = ResImagePack::loadImage(Path::dataReadPath("images/menu/numbers.png"));
...
{
  SDL_Surface *bgSurface = ResImagePack::loadImage(Path::dataReadPath("images/menu/map.png"));
  NodeDrawer drawer;
  drawer.setScreen(bgSurface);
  drawer.drawSelected(m_level->getLevelName());

  SolverDrawer solver(m_status);
  solver.setShift(V2((bgSurface->w - solver.getW()) / 2, bgSurface->h - 150));
...
void Pedometer::prepareRack()
{
  ...
  m_rack = new LayeredPicture(Path::dataReadPath("images/menu/pedometer.png"), V2(POS_X, POS_Y),
...
void Pedometer::drawNumbers(SDL_Surface *screen, int value)
{
  ...
  for (int i = CIPHERS - 1; i >= 0; --i)
  {
    ...
    int shiftY = max(numberHeight * (9 - cipher), numberHeight * 9 - SHIFT_SPEED * m_meterPhase);
    m_meterPhase++;
