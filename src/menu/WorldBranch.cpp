 ...
 */
LevelNode *WorldBranch::parseMap(const Path &datafile, LevelNode **outEnding, ResDialogPack *destPack)
{
  m_outPack = destPack;
  ...
  if (m_ending)
  {
    ...
    {
      throw LogicException(ExInfo("cannot export ending node").addInfo("ending", m_ending->getCodename()));
    }
  }
...
void WorldBranch::addDesc(const std::string &codename, LevelDesc *desc)
{
  ...
  {
    throw LogicException(ExInfo("cannot export level description").addInfo("codename", codename));
  }
}
//-----------------------------------------------------------------
/**
 ...
 */
void WorldBranch::addNode(const std::string &parent, LevelNode *new_node, bool hidden)
{
 ...
 */
void WorldBranch::bestSolution(const std::string &codename, int moves, const std::string &author)
{
...
{
  Path solved = Path::dataReadPath(LevelStatus::getSolutionFilename(codename));
