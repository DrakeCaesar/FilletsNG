...
//-----------------------------------------------------------------
WorldMap::WorldMap() : m_lastMouseLoc(-1, -1)
{
  m_selected = NULL;
  m_startNode = NULL;
  m_ending = NULL;
 ...
 */
void WorldMap::prepareBg()
{
  m_bg = new LayeredPicture(Path::dataReadPath("images/menu/map.png"), V2(0, 0),
 ...
 */
void WorldMap::initMap(const Path &mapfile)
{
  WorldBranch parser(NULL);
  m_startNode = parser.parseMap(mapfile, &m_ending, m_descPack);
  if (NULL == m_startNode)
  {
    throw LogicException(ExInfo("cannot create world map").addInfo("file", mapfile.getNative()));
  }
}
//-----------------------------------------------------------------
/**
...
void WorldMap::own_resumeState()
{
  ...
  SoundAgent::agent()->playMusic(Path::dataReadPath("music/menu.ogg"), NULL);
}
//-----------------------------------------------------------------
/**
 * Stop music.
 */
void WorldMap::own_cleanState()
{
...
void WorldMap::watchCursor()
{
  ...
  if (m_activeMask == m_maskIntro || m_activeMask == m_maskExit || m_activeMask == m_maskCredits ||
 ...
 */
void WorldMap::runSelected()
{
  Level *level = createSelected();
  if (level)
  {
    m_levelStatus->prepareRun(m_selected->getCodename(), m_selected->getPoster(), m_selected->getBestMoves(),
 ...
 */
Level *WorldMap::createSelected() const
{
  Level *result = NULL;
  if (m_selected)
  {
 ...
 */
bool WorldMap::checkEnding() const
{
  bool result = false;
  if (m_ending && m_selected != m_ending)
  {
...
//-----------------------------------------------------------------
std::string WorldMap::findLevelName(const std::string &codename) const
{
  std::string result;
  const LevelDesc *desc = dynamic_cast<const LevelDesc *>(m_descPack->findDialogHard(codename));
  if (desc)
  {
...
//-----------------------------------------------------------------
std::string WorldMap::findDesc(const std::string &codename) const
{
  std::string result;
  const LevelDesc *desc = dynamic_cast<const LevelDesc *>(m_descPack->findDialogHard(codename));
  if (desc)
...
void WorldMap::runIntro()
{
  ...
  LOG_WARNING(ExInfo("cannot find intro").addInfo("file", movieFile.getNative()));
#endif
...
//-----------------------------------------------------------------
void WorldMap::runCredits()
{
  pushState(new PosterScroller(Path::dataReadPath("images/menu/credits.png")));
}

//-----------------------------------------------------------------
void WorldMap::runOptions()
{
