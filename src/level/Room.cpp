 ...
 */
Room::Room(int w, int h, const std::string &picture, PhaseLocker *locker, Planner *levelScript)
{
  m_locker = locker;
...
Cube *Room::getModel(int model_index)
{
  ...
  {
    throw LogicException(ExInfo("bad model index").addInfo("model_index", model_index));
  }
...
bool Room::makeMove(char move)
{
  ...
  if (isFresh())
  {
    ...
    {
      throw LoadException(ExInfo("load error - bad move").addInfo("move", std::string(1, move)));
    }
    m_lastAction = Cube::ACTION_MOVE;
    result = true;
  }
...
void Room::playSound(const std::string &name, int volume)
{
  ...
  {
    SoundAgent::agent()->playSound(m_soundPack->getRandomRes(name), volume);
  }
}
//-----------------------------------------------------------------
/**
