...
void Anim::drawAt(SDL_Surface *screen, SDL_Renderer *renderer, int x, int y, eSide side)
{
  ...
  {
    SDL_Surface *surface = m_animPack[side]->getRes(m_animName, m_animPhase);
    ...
    {
      surface = m_animPack[side]->getRes(m_specialAnimName, m_specialAnimPhase);
      auto texture = SDL_CreateTextureFromSurface(renderer, surface);
      m_effect->blit(renderer, texture, surface, x, y);
      SDL_DestroyTexture(texture);
    }
...
void Anim::useSpecialAnim(const std::string &name, int phase)
{
  ...
  if (m_specialAnimPhase >= count)
  {
    ...
    LOG_WARNING(
        ExInfo("special anim phase over-flow").addInfo("anim", name).addInfo("phase", phase).addInfo("count", count));
  }
}
...
void Anim::changeEffect(ViewEffect *new_effect)
{
  if (NULL == new_effect)
  {
    throw LogicException(
        ExInfo("new_effect is NULL").addInfo("animName", m_animName).addInfo("specialAnimName", m_specialAnimName));
  }
...
void Anim::setEffect(const std::string &effectName)
{
  ...
  {
    ExInfo error = ExInfo("unknown view effect").addInfo("effect", effectName);
    LOG_WARNING(error);
  }
}
...
//-----------------------------------------------------------------
static int decodeInt(const std::string &input)
{
  bool ok;
  int result = (int)StringTool::readInt(input.c_str(), &ok);
  if (!ok)
  {
    LOG_WARNING(ExInfo("invalid int").addInfo("input", input));
  }
  return result;
}

//-----------------------------------------------------------------
std::string Anim::getState() const
{
  std::string output;
...
void Anim::restoreState(const std::string &state)
{
  ...
  if (values.size() != 8)
  {
    LOG_WARNING(ExInfo("invalid anim state").addInfo("state", state));
    return;
  }
