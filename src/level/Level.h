...
protected:
	...
	void own_noteBg() override;
	void own_noteFg() override;

public:
	Level(const std::string& codename, const Path& datafile, int depth);
	~Level() override;
	const char* getName() const override { return "state_level"; };
	...
	int getCountForSolved() const override;
	int getCountForWrong() const override;
};
