...
//-----------------------------------------------------------------
inline LevelStatus *getStatus(lua_State *L)
{
...
//-----------------------------------------------------------------
void LevelStatus::prepareRun(const std::string &codename, const std::string &poster, int bestMoves,
...
//-----------------------------------------------------------------
std::string LevelStatus::getSolutionFilename(const std::string &codename)
{
  return "solved/" + codename + ".lua";
}
//-----------------------------------------------------------------
std::string LevelStatus::getSolutionFilename() const
{
...
void LevelStatus::writeSolvedMoves(const std::string &moves)
{
  ...
  if (prevMoves.empty() || moves.size() < prevMoves.size())
  {
    ...
    {
      LOG_WARNING(ExInfo("cannot save solution").addInfo("file", file.getNative()).addInfo("moves", moves));
    }
  }
}
//-----------------------------------------------------------------
