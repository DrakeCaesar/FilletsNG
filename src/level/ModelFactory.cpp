 ...
 */
Cube *ModelFactory::createModel(const std::string &kind, const V2 &loc, const std::string &shape)
{
  ...
  Cube *model = new Cube(loc, weight, power, alive, newShape);
 ...
 */
void ModelFactory::createParams(const std::string &kind, Cube::eWeight *out_weight, Cube::eWeight *out_power,
                                bool *out_alive)
{
  if ("fish_small" == kind)
  {
  ...
  else
  {
    ...
    *out_alive = false;
    ...
    {
      throw LogicException(ExInfo("unknown model kind").addInfo("kind", kind));
    }
  }
}
//-----------------------------------------------------------------
 ...
 */
Unit *ModelFactory::createUnit(const std::string &kind)
{
  Unit *result = NULL;
  if ("fish_small" == kind)
  {
  ...
  else if (StringTool::startsWith(kind, "fish_extra") || StringTool::startsWith(kind, "fish_EXTRA"))
  {
 ...
 */
Cube *ModelFactory::createBorder()
{
  Cube *border = new Cube(V2(-1, -1), Cube::FIXED, Cube::NONE, false, new Shape("X\n"));
  return border;
}
//-----------------------------------------------------------------
/**
 ...
 */
Cube *ModelFactory::createOutputItem(const std::string &kind, const V2 &loc, const std::string &shape)
{
  ...
  {
    throw LogicException(ExInfo("unknown border dir").addInfo("kind", kind));
  }

  Cube *model = new Cube(loc, Cube::FIXED, Cube::NONE, false, new Shape(shape));
 ...
 */
ControlSym ModelFactory::parseExtraControlSym(const std::string &kind)
{
  static const std::string PREFIX = "fish_extra-";
  if (kind.size() != PREFIX.size() + 4)
  {
    throw LogicException(ExInfo("you must specify control symbols").addInfo("kind", kind));
  }
