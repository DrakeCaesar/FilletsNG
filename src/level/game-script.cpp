...
//-----------------------------------------------------------------
inline LevelScript *getLevelScript(lua_State *L)
{
  return dynamic_cast<LevelScript *>(script_getLeader(L));
}

//-----------------------------------------------------------------
inline Cube *getModel(lua_State *L, int model_index)
{
...
int script_game_addDecor(lua_State *L) throw()
{
  ...
  if ("rope" == decor_name)
  {
    ...
    getLevelScript(L)->room()->addDecor(new RopeDecor(model1, model2, V2(shift_x1, shift_y1), V2(shift_x2, shift_y2)));
  }
  else
  {
    LOG_WARNING(ExInfo("unknown decor").addInfo("decor_name", decor_name));
  }
...
int script_model_addAnim(lua_State *L) throw()
{
  ...
  Anim::eSide lookDir = static_cast<Anim::eSide>(luaL_optinteger(L, 4, Anim::SIDE_LEFT));

  Cube *model = getModel(L, model_index);
  if (picture[0] == '\0')
  {
    model->anim()->addAnim(anim_name, ShapeBuilder::createImage(model->shape(), model->getWeight()), lookDir);
  }
  else
  {
...
int script_model_setGoal(lua_State *L) throw()
{
  ...
  {
    ExInfo error = ExInfo("unknown goal").addInfo("goal", goalname);
    LOG_WARNING(error);
