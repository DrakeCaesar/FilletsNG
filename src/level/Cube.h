...
public:
  ...
  {
    return m_goal.isSatisfy(this);
  }

  bool isWrong() const
  {
    return m_goal.isWrong(this);
  }

  bool isWall() const
  {
    return m_weight >= Cube::FIXED;
  }

  bool shouldGoOut() const
  {
    return m_goal.shouldGoOut();
  }
  ...
  int getOutCapacity()
  {
    return m_outCapacity;
  }

  void setOutDir(Dir::eDir outDir, int capacity = 2, eWeight weight = Cube::FIXED);
