...
//-----------------------------------------------------------------
Outline::Outline(const SDL_Color &color, int width) : m_color(color)
{
  m_width = width;
  m_pixel = 0;
}
...
void Outline::fillNeighbourhood(SDL_Surface *surface, Uint32 bgKey, int x, int y)
{
  ...
  if (x + 1 < surface->w && PixelTool::getPixel(surface, x + 1, y) == bgKey)
  {
    PixelTool::putPixel(surface, x + 1, y, m_pixel);
  }
  if (y + 1 < surface->h && PixelTool::getPixel(surface, x, y + 1) == bgKey)
  {
    PixelTool::putPixel(surface, x, y + 1, m_pixel);
  }
}
