 ...
 */
LayeredPicture::LayeredPicture(const Path &bg_file, const V2 &loc, const Path &lowerLayer, const Path &colorMask)
    : Picture(bg_file, loc)
{
  ...
  if (m_lowerLayer->w != m_colorMask->w || m_lowerLayer->h != m_colorMask->h)
  {
    ...
    throw ResourceException(ExInfo("lowerLayer and colorMask have different proportions")
...
Uint32 LayeredPicture::getMaskAt(const V2 &loc)
{
  ...
  if ((0 <= loc.getX() && loc.getX() < m_colorMask->w) && (0 <= loc.getY() && loc.getY() < m_colorMask->h))
  {
    ...
    result = PixelTool::getPixel(m_colorMask, loc.getX(), loc.getY());
  }
  return result;
}
...
void LayeredPicture::drawOn(SDL_Surface *screen, SDL_Renderer *renderer)
{
  ...
  // Since SDL does not provide a built-in way to do this, you might need to use a custom shader with
  // SDL_RenderReadPixels or similar
