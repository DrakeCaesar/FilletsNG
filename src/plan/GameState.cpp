 ...
 */
const InputProvider *GameState::getInput()
{
  return m_handler;
}
//-----------------------------------------------------------------
...
void GameState::updateState()
{
  if (!m_active)
  {
    throw LogicException(ExInfo("update - state is not active").addInfo("name", getName()));
  }
...
void GameState::pauseState()
{
  if (!m_active)
  {
    throw LogicException(ExInfo("pause - state is not active").addInfo("name", getName()));
  }
...
void GameState::resumeState()
{
  if (m_active)
  {
    throw LogicException(ExInfo("resume - state is already active").addInfo("name", getName()));
  }
  m_active = true;
  own_resumeState();
}
...
void GameState::cleanState()
{
  ...
  if (!m_active)
  {
    throw LogicException(ExInfo("clean - state is not active").addInfo("name", getName()));
  }
  own_cleanState();
  unHandlers();
