 ...
 */
void SubTitleAgent::newSubtitle(const std::string &original, const std::string &fontname)
{
 ...
 */
std::string SubTitleAgent::splitAndCreate(const std::string &original, const Color *color)
{
  std::string subtitle = original;
...
void SubTitleAgent::trimRest(std::string &buffer)
{
  ...
  if (i <= 0)
  {
    LOG_WARNING(ExInfo("unbreakable string").addInfo("string", buffer));
    if (buffer.size() > 4)
    {
...
//-----------------------------------------------------------------
void SubTitleAgent::newShortSubtitle(const std::string &subtitle, const Color *color)
{
  int startY = lowestY();
  int finalY = TITLE_BASE + TITLE_ROW;
  int bonusTime = (TITLE_BASE - startY + m_limitY - TITLE_LIMIT_Y) / TITLE_SPEED;
  Title *title = new Title(startY, finalY, bonusTime, m_limitY, subtitle, m_font, color);
