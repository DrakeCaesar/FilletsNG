...
void StateManager::popState(GameState *who)
{
  ...
  {
    MessagerAgent::agent()->forwardNewMsg(new SimpleMsg(Name::APP_NAME, "quit"));
  }
}
 ...
 */
StateManager::t_states::iterator StateManager::findIter(GameState *who)
{
  t_states::iterator end = m_states.end();
  for (t_states::iterator i = m_states.begin(); i != end; ++i)
  {
  ...
  throw LogicException(ExInfo("game state is not found in stack").addInfo("state", who ? who->getName() : "(null)"));
}
//-----------------------------------------------------------------
/**
